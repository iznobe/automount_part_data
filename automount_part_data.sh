...
while true; do
  read -rp "Voulez-vous utiliser le dossier « /media » , « /mnt » , ou « /run/media » pour monter la partition ?
  choix : [ 1 / 2 / 3 / A ] ?"
  case "$REPLY" in
  1)
    Mount="/mnt"
...
nbDev=$(("${#ListPart[@]}"/5))
  ...
  while true; do
    ...
    Y|y|O|o|"")
      ...
      if test "$do_change" = "yes"; then
        ...
        chown -v "$SUDO_USER": "$trash_user_dir" && chmod -v 700 "$trash_user_dir"
  ...
  while true; do
    ...
    Lancer le déplacement des données maintenant ?  [O/n]" rep_deplace
    case "$rep_deplace" in
    N|n)
      info "Pas de déplacement des données , elles sont déja sur la partition « $part_data_user_dir »"
      deplace=false
      break
      ;;
    Y|y|O|o|"")
  ...
  if test "$REPLY" == "2" -a "$REPLY" == "3"; then
    printf "\n"
    info "création des liens symboliques :"
    printf "\n"
    for dir_name in "${dir_tab[@]}"; do
      sudo -u "$SUDO_USER" ln -s "$part_data_user_dir/$dir_name" "$home"
    done
  fi

  if test "$REPLY" == "1" -a "$REPLY" == "3"; then
    printf "\n"
    ...
    for dir_name in "${dir_tab[@]}"; do
      ...
      if test "$REPLY" != "2"; then # traitement XDG
